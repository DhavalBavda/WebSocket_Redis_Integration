...
//   await redisSubscriber.connect();
  ...
  //   console.log(`Server 1: Received message for room ${room}: ${content}`);
  ...
  //   console.log(`Server 1: Client connected - ${socket.id}`);

  //   // Handle room join
  //   socket.on('joinRoom', (room) => {
  //     socket.join(room);
  //     console.log(`Server 1: Client ${socket.id} joined room ${room}`);
  //   });
...
app.post('/sendNotification',async (req,res)=>{
    try {
      ...
      await redisPublisher.publish("notifcation", message);
      res.json("200");
    } catch (error) {
      console.log(error);
    }
...
httpServer.listen(3001, () => {
  console.log('Server 1 listening on http://localhost:3001');
});